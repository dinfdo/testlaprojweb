...
if ($method === 'GET') {
    ...
    if ($resource === 'components') {
        $stmt = $pdo->query('
            SELECT c.id, c.name, c.description, c.purpose AS category, c.difficulty_level,
                   c.image_path AS image_url, c.slot_position, d.id AS device_id, d.name AS device_name
    ...
    if ($resource === 'levels') {
        $stmt = $pdo->query('
            ...
            FROM levels l JOIN devices d ON d.id = l.device_id
...
if ($method === 'POST') {
    if ($resource === 'component') {
        ...
        $stmt = $pdo->prepare('
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ');
        $stmt->execute([$deviceId, $name, $description, $purpose, $difficulty, $imagePath, $slotPosition]);
    ...
    if ($resource === 'level') {
        ...
        $gameType    = $body['game_type']                  ?? 'quiz';
        $minScore    = isset($body['min_score_required'])  ? (int)$body['min_score_required']  : 0;

        if (!$deviceId || $levelNumber < 1 || !$title || $minScore < 0) {
            json_response(['success' => false, 'message' => 'Missing or invalid fields'], 400);
        }
        if (!in_array($gameType, ['learn', 'drag_drop', 'matching', 'quiz'], true)) {
            json_response(['success' => false, 'message' => 'Invalid game_type'], 400);
        }

        $stmt = $pdo->prepare('
            INSERT INTO levels (device_id, level_number, title, description, game_type, min_score_required)
            VALUES (?, ?, ?, ?, ?, ?)
        ');
        $stmt->execute([$deviceId, $levelNumber, $title, null, $gameType, $minScore]);
        json_response(['success' => true, 'id' => (int)$pdo->lastInsertId()]);
    }
